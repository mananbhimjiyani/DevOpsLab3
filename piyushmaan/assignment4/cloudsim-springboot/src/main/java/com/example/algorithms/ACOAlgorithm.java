...
//    }
    ...
    private Vm selectVm(Ant ant, double[][] pheromoneMatrix) {
        ...
        for (int j = 0; j < vmList.size(); j++) {
            double heuristic = 1.0 / vmList.get(j).getMips(); // Higher MIPS = lower execution time
            probabilities[j] = pheromoneMatrix[cloudletIndex][j] * heuristic;
            totalPheromone += probabilities[j];
        }

        // Normalize probabilities
        for (int j = 0; j < probabilities.length; j++) {
            probabilities[j] /= totalPheromone;
        }

        // Select VM based on probability distribution
        ...
        return vmList.get(0); // Fallback (should not happen)
    }
